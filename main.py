...
app = Flask('')
@app.route('/')
def home(): return "Bot Aktif ve Uyanik!"
def run_flask(): 
    port = int(os.environ.get("PORT", 8000))
    app.run(host='0.0.0.0', port=port)

# --- BOT AYARLARI (YENI TOKEN) ---
TOKEN = '8122282591:AAFLcVGOtv48TaFRrIPcU8Dg0n3DCwSICnY'
bot = telebot.TeleBot(TOKEN)

# --- DOSYADAN HESAP ÇEKME ---
...
def start(m):
    ...
    msg = (
        f"✅ **BOT TÜM SİSTEMLERİYLE AKTİF!**\n\n"
        f"💎 **Mevcut Hakkın:** {credits}\n"
        f"🔗 **Ref Linkin:** `{ref_link}`\n\n"
        f"🎬 **Hesap Almak İçin Komutlar:**\n"
        f"/netflix | /spotify | /exxen\n"
        f"/hbomax | /predunyam"
    )
...
def process_req(m, file, service):
    ...
    if u.data and u.data[0]['credits'] > 0:
        ...
        if acc:
            ...
            bot.send_message(m.chat.id, f"🚀 **İste {service} Hesabın:**\n\n`{acc}`\n\nKalan Hakkın: **{new_c}**", parse_mode="Markdown")
        else: bot.send_message(m.chat.id, f"⚠️ Üzgünüm, {service} stokları şu an boş.")
    else: bot.send_message(m.chat.id, "⚠️ Hakkın kalmamış! Arkadaşlarını davet et.")
